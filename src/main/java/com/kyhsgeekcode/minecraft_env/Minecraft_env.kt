...
class Minecraft_env : ModInitializer, CommandExecutor {
    ...
    override fun onInitialize() {
        ...
        ClientTickEvents.START_WORLD_TICK.register(ClientTickEvents.StartWorldTick { world: ClientWorld ->
            ...
            if (client.isPaused) return@StartWorldTick
            ...
            val player = client.player ?: return@StartWorldTick
            initializer.onWorldTick(client.inGameHud.chatHud, player, this)
            if (!initializer.initWorldFinished || isResetting) {
                println("Waiting for world init ticks: $isResetting")
                isResetting = true
                return@StartWorldTick
            }
...
//                System.out.println("Waiting for command");
                ...
                if (command != null && command.isNotEmpty()) {
                    if (command == "respawn") {
                        ...
                        println("Player is dead, respawn")
                ...
                if (player.isDead) return@StartWorldTick else client.setScreen(null)
...
//                        });
    ...
    private fun sendObservation(writer: OutputStreamWriter, world: World, initializer: EnvironmentInitializer) {
        ...
        if (isResetting && initializer.initWorldFinished) { // reset finished
            isResetting = false
